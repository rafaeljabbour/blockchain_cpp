...
Block Block::Deserialize(const std::vector<uint8_t> &serialized) {
  ...
  offset += 4;
  block.data = std::vector<uint8_t>(serialized.begin() + offset, serialized.begin() + offset + dataSize);

  offset += dataSize;
  block.previousHash = std::vector<uint8_t>(serialized.begin() + offset, serialized.begin() + offset + 32);

  offset += 32;
  block.hash = std::vector<uint8_t>(serialized.begin() + offset, serialized.begin() + offset + 32);
